...
function renderSentenceRow(sentence, compact = false) {
  ...
  const notes = sortedAnnotations(annotationsForSentence(sentence.id).filter((annotation) => annotation.type !== "target" && ((ui.showAnnotationDetails && rememberedText(annotation) !== annotation.selectedText) || (ui.showAnnotationNotes && annotation.note)))).map((annotation) => `<div class="annotation-summary ${annotation.type}"><span class="annotation-pill ${annotation.type}">${ANNOTATION_TYPES[annotation.type]}</span><strong>${escapeHTML(rememberedText(annotation))}</strong>${ui.showAnnotationNotes && annotation.note ? `<p>${escapeHTML(annotation.note)}</p>` : ""}</div>`).join("");
...
function renderSentenceControls(compact = false) {
  ...
  if (compact) return `<div class="sentence-controls compact"><button class="control-pill ${ui.showAnnotations || ui.showAnnotationDetails || ui.showAnnotationNotes || ui.showSentenceNotes ? "active" : ""}" data-action="display-options">显示</button><button class="control-pill" data-action="sort-options">排序</button></div>`;
...
function renderSheet() {
  ...
  if (sheet.kind === "display-options") { const isAll = collectionFor(ui.collectionId)?.isAll; return standardChoiceSheet([{ action: "display-mode", value: "all", label: "显示全部", checked: ui.showAnnotations && ui.showAnnotationDetails && ui.showAnnotationNotes && ui.showSentenceNotes && (!isAll || ui.showCollectionNames) }, { action: "display-mode", value: "none", label: "隐藏全部", checked: !ui.showAnnotations && !ui.showAnnotationDetails && !ui.showAnnotationNotes && !ui.showSentenceNotes && (!isAll || !ui.showCollectionNames) }, { action: "display-mode", value: "annotations", label: "句子标注", checked: ui.showAnnotations }, { action: "display-mode", value: "annotation-details", label: "标注扩展", checked: ui.showAnnotationDetails }, { action: "display-mode", value: "annotation-notes", label: "笔记", checked: ui.showAnnotationNotes }, { action: "display-mode", value: "sentence-notes", label: "句子笔记", checked: ui.showSentenceNotes }, ...(isAll ? [{ action: "display-mode", value: "collection-names", label: "显示合集名称", checked: ui.showCollectionNames }] : [])]); }
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "hide-word-details") { ui.wordExampleDisplay = "none"; ui.wordShowPhrases = false; ui.wordShowSynonyms = false; ui.sheet = null; render(); }
  ...
  if (action === "display-mode") {
    ...
    if (mode === "all") { ui.showAnnotations = ui.showAnnotationDetails = ui.showAnnotationNotes = ui.showSentenceNotes = true; if (collectionFor(ui.collectionId)?.isAll) ui.showCollectionNames = true; }
    if (mode === "none") { ui.showAnnotations = ui.showAnnotationDetails = ui.showAnnotationNotes = ui.showSentenceNotes = false; if (collectionFor(ui.collectionId)?.isAll) ui.showCollectionNames = false; }
    ...
    if (mode === "annotation-notes") ui.showAnnotationNotes = !ui.showAnnotationNotes;

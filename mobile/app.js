...
const DB_VERSION = 1;
const STORES = ["sentences", "annotations", "entries", "links", "collections", "settings"];
...
let model = { sentences: [], annotations: [], entries: [], links: [], collections: [], settings: [] };
let ui = { screen: "reading", collectionId: null, wordbookCollectionId: null, wordbookReturn: "reading", wordbookType: "all", wordbookSort: "recent", wordbookShowOriginal: true, wordbookShowNotes: true, wordsLibraryOpen: false, wordsLibraryTab: "reading", wordsLibraryType: "all", wordCollectionId: "words-all", wordExampleDisplay: "one", sentenceId: null, entryId: null, search: "", sort: "recent", sentenceSort: "recent", allCollectionSort: "recent", annotationSort: "recent", showAnnotations: true, showNotes: true, showAnnotationDetails: true, showSentenceNotes: true, showCollectionNames: true, bulkEditing: false, bulkSelection: [], sheet: null, selection: null, tokenSelection: [], appendAnnotationId: "", toast: "" };
let toastTimer;
...
function wordCollections() { return model.collections.filter((item) => item.scope === "words").sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0)); }
function wordCollectionFor(idValue) { return wordCollections().find((item) => item.id === idValue); }
...
function sentenceMarkup(sentence, { interactive = true, visible = true, selectable = false } = {}) {
  if (!visible) return escapeHTML(sentence.text);
  const annotations = annotationsForSentence(sentence.id).filter((item) => annotationRanges(item).some((range) => range.startOffset >= 0 && range.endOffset <= sentence.text.length));
...
function renderReading() {
  ...
  return `${header("Reading")}<section class="page-content">${renderSearch("Search saved sentences")}<section class="collection-list">${rootCollections().map((collection) => renderCollectionCard(collection)).join("")}</section></section>`;
}
...
function renderCollectionCard(collection) {
  ...
  return `<button class="collection-card" data-action="open-collection" data-id="${collection.id}"><span class="folder-mark">⌁</span><span><strong>${escapeHTML(collection.name)}</strong><small>${directCount + children.reduce((total, child) => total + readingSentences().filter((sentence) => sentence.collectionId === child.id).length, 0)} sentences${children.length ? ` · ${children.length} sections` : ""}</small></span><i>›</i></button>`;
}
...
function renderWordbook() {
  ...
  return `${header("单词本", "back-wordbook")}<section class="page-content wordbook-page"><div class="wordbook-toolbar"><strong>${escapeHTML(collection.name)}</strong><div><button class="control-pill" data-action="wordbook-display-options">显示</button><button class="control-pill" data-action="wordbook-sort-options">排序</button></div></div><div class="wordbook-filters">${filters.map(([type, label]) => `<button class="${ui.wordbookType === type ? "active" : ""} ${type}" data-action="wordbook-filter" data-value="${type}">${label}</button>`).join("")}</div>${annotations.length || sentenceNotes.length ? `<div class="annotation-list wordbook-list">${annotations.map((annotation) => { const sentence = sentenceFor(annotation.sentenceId); return `<button data-action="annotation-detail" data-id="${annotation.id}"><span class="annotation-pill ${annotation.type}">${ANNOTATION_TYPES[annotation.type]}</span><strong>${escapeHTML(rememberedText(annotation))}</strong>${ui.wordbookShowOriginal ? `<small class="wordbook-source">${escapeHTML(sentence?.text || "")}</small>` : ""}${ui.wordbookShowNotes && annotation.note ? `<small class="wordbook-note">${escapeHTML(annotation.note)}</small>` : ""}<i>›</i></button>`; }).join("")}${sentenceNotes.map((sentence) => `<button data-action="sentence-note" data-id="${sentence.id}"><span class="annotation-pill note">笔记</span><strong>${escapeHTML(sentence.note)}</strong>${ui.wordbookShowOriginal ? `<small class="wordbook-source">${escapeHTML(sentence.text)}</small>` : ""}<i>›</i></button>`).join("")}</div>` : `<div class="quiet-empty">This collection has no marked items yet.</div>`}</section>`;
}
...
function renderWords() {
  ...
  const currentCollectionName = ui.wordCollectionId === "words-all" ? "全部" : ui.wordCollectionId === "words-uncategorized" ? "未分类" : wordCollectionFor(ui.wordCollectionId)?.name || "全部";
  const pinned = wordCollections().filter((collection) => collection.showOnWordsHome || collection.id === ui.wordCollectionId);
  const filters = [{ id: "words-all", name: "全部" }, ...pinned, ...(ui.wordCollectionId === "words-uncategorized" ? [{ id: "words-uncategorized", name: "未分类" }] : [])];
  return `${header("Words")}<section class="page-content"><button class="word-collection-display" data-action="word-collection-switcher"><strong>${escapeHTML(currentCollectionName)}</strong><span aria-hidden="true">⌄</span></button><div class="words-actions"><div><button class="text-action" data-action="open-words-library">单词本</button><button class="text-action" data-action="new-entry">＋ 新增</button></div><div><button class="control-pill ${ui.wordExampleDisplay !== "none" ? "active" : ""}" data-action="words-display-options">显示</button><button class="control-pill" data-action="words-sort-options">排序</button></div></div>${filters.length > 1 || ui.wordCollectionId !== "words-all" ? `<div class="word-collection-filters">${filters.map((collection) => `<button class="${ui.wordCollectionId === collection.id ? "active" : ""}" data-action="filter-word-collection" data-id="${collection.id}">${escapeHTML(collection.name)}</button>`).join("")}</div>` : ""}${entries.length ? `<div class="entry-list">${sorted(entries).map(renderEntryRow).join("")}</div>` : empty(ui.wordCollectionId === "words-all" ? "Add a word, then collect examples that make it stick." : "这个合集里还没有词条。", "new-entry", "＋ 新增")}</section>`;
}
...
function renderEntryRow(entry) {
  ...
  const visibleExamples = ui.wordExampleDisplay === "all" ? examples : ui.wordExampleDisplay === "one" ? examples.slice(0, 1) : [];
  return `<button class="entry-row" data-action="open-entry" data-id="${entry.id}"><span class="entry-dot"></span><strong>${escapeHTML(entry.term)}</strong>${visibleExamples.length ? `<span class="entry-examples">${visibleExamples.map((example) => `<small class="entry-example">${sentenceMarkup(example, { interactive: false })}</small>`).join("")}</span>` : ""}<i>›</i></button>`;
}
...
function renderLibraryRow(collection, mode) {
  ...
  return `<button class="library-list-row" data-action="${action}" data-id="${collection.id}"><span class="library-list-icon ${mode}">⌁</span><span class="library-list-copy"><strong>${escapeHTML(collection.name)}</strong><small>${count} ${suffix}</small></span><i>›</i></button>`;
}
...
function renderWordsLibrary() {
  const wordCollectionsForLibrary = [{ id: "words-all", name: "全部" }, ...wordCollections(), { id: "words-uncategorized", name: "未分类" }];
  return `${header("单词本", "back-words-library", `<span></span>`)}<section class="page-content words-library-page">${renderWordsLibraryBlock("Word", wordCollectionsForLibrary, "word")}${renderWordsLibraryBlock("Reading", rootCollections(), "reading")}</section>`;
}
...
function renderEntry() {
  ...
  return `${header(entry.term, "back-words")}<section class="page-content entry-detail"><div class="entry-hero"><div><p>${ENTRY_TYPES[entry.kind] || "Other"}</p><h2>${escapeHTML(entry.term)}</h2>${entry.hint ? `<small>${escapeHTML(entry.hint)}</small>` : ""}<button class="entry-collection-label" data-action="entry-collection" data-id="${entry.id}">合集 · ${escapeHTML(wordCollectionName(entry))} ›</button></div><button class="speak-button" data-action="speak" data-term="${escapeHTML(entry.term)}" aria-label="Play ${escapeHTML(entry.term)}">⌁</button></div><button class="wide-add" data-action="add-example" data-id="${entry.id}">＋ Add Example</button><section class="section-head"><h2>Examples</h2><span>${sentences.length}</span></section>${sentences.length ? `<div class="sentence-list">${sorted(sentences).map((sentence) => renderSentenceRow(sentence)).join("")}</div>` : empty("The examples you choose will live here.", "add-example", "Add the first example", entry.id)}<button class="danger-link" data-action="delete-entry" data-id="${entry.id}">Delete entry</button></section>`;
}
...
function renderSentence() {
  ...
  const selectionTypes = exampleEntry ? [["target", "词条"], ["word", "生词"], ["phrase", "短语"], ["context", "语境"]] : Object.entries(ANNOTATION_TYPES).filter(([type]) => type !== "target");
  ...
  return `${header(exampleEntry ? "Example" : "Sentence", "back-from-sentence", `<span></span>`)}<section class="page-content sentence-detail">${actions}<article class="sentence-paper"><div class="sentence-body" data-sentence-body="${sentence.id}">${sentenceMarkup(sentence, { selectable: true })}</div></article><p class="selection-help">Tap words directly. Select adjacent or separate words.</p>${selectionTools}<section class="section-head"><h2>Annotations</h2><span>${annotations.length}</span></section>${annotationRows || sentenceNote ? `<div class="annotation-list">${annotationRows}${sentenceNote}</div>` : `<div class="quiet-empty">Nothing needs to become a task. Mark only what matters.</div>`}</section>`;
}
...
function renderSheet() {
  ...
  if (sheet.kind === "words-display-options") return `<div class="scrim" data-action="close-sheet"></div><section class="sheet quick-sheet"><button data-action="set-word-example-display" data-value="one"><b>显示 1 条例句${ui.wordExampleDisplay === "one" ? " ✓" : ""}</b><span>每个词条显示最近添加的一条例句。</span></button><button data-action="set-word-example-display" data-value="all"><b>显示全部例句${ui.wordExampleDisplay === "all" ? " ✓" : ""}</b><span>在词表中展开每个词条的所有例句。</span></button><button data-action="set-word-example-display" data-value="none"><b>隐藏例句${ui.wordExampleDisplay === "none" ? " ✓" : ""}</b><span>只显示词条名称。</span></button><button data-action="close-sheet">取消</button></section>`;
  if (sheet.kind === "word-collection-switcher") { const choices = [{ id: "words-all", name: "全部" }, ...wordCollections(), { id: "words-uncategorized", name: "未分类" }]; return `<div class="scrim" data-action="close-sheet"></div><section class="sheet quick-sheet"><div class="sheet-choice-title">选择合集</div>${choices.map((collection) => `<button data-action="set-current-word-collection" data-id="${collection.id}"><b>${escapeHTML(collection.name)}${ui.wordCollectionId === collection.id ? " ✓" : ""}</b><span>${entriesForWordCollection(collection.id).length} 个词条</span></button>`).join("")}<button data-action="new-word-collection"><b>＋ 新建合集</b></button><button data-action="close-sheet">取消</button></section>`; }
  if (sheet.kind === "words-sort-options") return `<div class="scrim" data-action="close-sheet"></div><section class="sheet quick-sheet"><button data-action="set-words-sort" data-value="recent"><b>最新在前${ui.sort === "recent" ? " ✓" : ""}</b><span>最近添加的词条排在上面。</span></button><button data-action="set-words-sort" data-value="oldest"><b>最早在前${ui.sort === "oldest" ? " ✓" : ""}</b><span>最早添加的词条排在上面。</span></button><button data-action="set-words-sort" data-value="alpha"><b>A–Z${ui.sort === "alpha" ? " ✓" : ""}</b><span>按词条字母顺序排列。</span></button><button data-action="set-words-sort" data-value="random"><b>随机${ui.sort === "random" ? " ✓" : ""}</b><span>打乱顺序，适合复习。</span></button><button data-action="close-sheet">取消</button></section>`;
  ...
  if (sheet.kind === "entry") {
    ...
    const selectedCollectionId = wordCollectionFor(ui.wordCollectionId)?.id || "";
    ...
    return `<div class="scrim" data-action="close-sheet"></div><section class="sheet form-sheet"><header><h2>${annotation ? "加入 Words" : "新增词条"}</h2><button data-action="close-sheet">×</button></header><form id="entry-form" data-annotation-id="${annotation?.id || ""}"><label>单词或短语 <input name="term" required value="${escapeHTML(suggested)}" placeholder="cheek / out of control" /></label><label>类型 <select name="kind">${Object.entries(ENTRY_TYPES).map(([key, label]) => `<option value="${key}" ${annotation?.type === key || (!annotation && key === "word") ? "selected" : ""}>${label}</option>`).join("")}</select></label><label>合集 <select name="collectionId">${collectionOptions}</select></label><label>提示 <input name="hint" placeholder="可选 · 默认隐藏" /></label><button class="primary-button" type="submit">保存词条</button></form></section>`;
  }
  if (sheet.kind === "word-collection") return `<div class="scrim" data-action="close-sheet"></div><section class="sheet form-sheet"><header><h2>新建词条合集</h2><button data-action="close-sheet">×</button></header><form id="word-collection-form"><label>合集名称 <input name="name" required autofocus placeholder="例如：旅行" /></label><label class="checkbox-label"><input type="checkbox" name="showOnWordsHome" checked /> 在 Words 首页显示</label><button class="primary-button" type="submit">创建合集</button></form></section>`;
  ...
  if (sheet.kind === "annotation") {
    ...
    return `<div class="scrim" data-action="close-sheet"></div><section class="sheet form-sheet annotation-sheet"><header><div class="annotation-title"><span class="annotation-pill ${annotation?.type}">${ANNOTATION_TYPES[annotation?.type]}</span><h2>${escapeHTML(rememberedText(annotation || {}) || "Annotation")}</h2></div><button data-action="close-sheet">×</button></header><form id="annotation-form" data-id="${annotation?.id || ""}"><label>Base form <input name="entryText" value="${escapeHTML(canonicalValue)}" placeholder="headsets → headset / yelling at us → yell at sb" /></label><label>Note <textarea name="note" placeholder="Optional · a small reminder">${escapeHTML(annotation?.note || "")}</textarea></label><button class="primary-button" type="submit">Save</button></form>${linked ? `<button class="sheet-link" data-action="open-entry" data-id="${linked.entryId}">In library · ${escapeHTML(entryFor(linked.entryId)?.term || "")}</button>` : `<button class="sheet-link" data-action="promote-annotation" data-id="${annotation?.id || ""}">＋ Add to Library</button>`}<button class="sheet-danger" data-action="delete-annotation" data-id="${annotation?.id || ""}">Delete annotation</button></section>`;
  }
  if (sheet.kind === "settings") return `<div class="scrim" data-action="close-sheet"></div><section class="sheet settings-sheet"><header><h2>Data</h2><button data-action="close-sheet">×</button></header><p>Your study data stays in this browser unless you export it.</p><button class="sheet-link" data-action="export-backup">Export backup</button><button class="sheet-link" data-action="import-backup">Import backup</button><p class="small-copy">Export regularly to Files or iCloud Drive. Clearing Safari website data will erase local data.</p></section>`;
  return "";
}

function render() {
  let content = ui.sentenceId ? renderSentence() : ui.screen === "words" ? renderWords() : renderReading();
  document.getElementById("app").innerHTML = `<div class="app-shell">${content}${renderNav()}${renderSheet()}${ui.toast ? `<div class="toast">${escapeHTML(ui.toast)}</div>` : ""}</div>`;
}
...
async function saveEntry(form) {
  ...
  if (!entry) entry = { id: id("entry"), term, normalizedTerm: normalized, kind, hint: form.hint.value.trim(), collectionId, createdAt: now(), updatedAt: now() };
  else entry = { ...entry, term, hint: form.hint.value.trim() || entry.hint || "", collectionId, updatedAt: now() };
...
document.addEventListener("click", async (event) => {
  ...
  if (action === "toggle-word-collection-home") {
    ...
    if (collection) { await db.put("collections", { ...collection, showOnWordsHome: !collection.showOnWordsHome, updatedAt: now() }); ui.sheet = null; await load(); flash(collection.showOnWordsHome ? "已从首页隐藏" : "已在首页显示"); }
  }
  ...
  if (action === "open-entry") { ui.entryId = recordId; ui.sentenceId = null; ui.sheet = null; ui.screen = "words"; render(); }
  ...
  if (action === "toggle-token") {
    ...
    const existing = (ui.tokenSelection || []).filter((item) => item.sentenceId === token.sentenceId);
    ...
    ui.selection = null; ui.tokenSelection = existing; render();
  }
  ...
  if (action === "promote-annotation") { ui.sheet = { kind: "entry", annotationId: recordId }; render(); }
  ...
  if (action === "delete-entry") { if (!window.confirm("Delete this entry? Its sentences will stay saved.")) return; await db.remove("entries", recordId); await Promise.all(linksForEntry(recordId).map((item) => db.remove("links", item.id))); ui.entryId = null; await load(); flash("Entry deleted"); }
  if (action === "delete-word-collection") {
    const collection = wordCollectionFor(recordId);
    if (!collection || !window.confirm(`删除「${collection.name}」？其中的词条会移到未分类。`)) return;
...
document.addEventListener("input", (event) => { if (event.target.id === "search") { ui.search = event.target.value; render(); document.getElementById("search")?.focus(); } });
